...
| `survey shown`, `survey dismissed`, `survey sent` with `$survey_id` | Visitor opens, dismisses, or submits a manually rendered PostHog API survey, or submits one of the feedback forms mirrored to the PostHog Feedback API survey. | Whether expectations before use, outcomes after use, and open-ended feedback are collected in PostHog Surveys, not only custom feedback events. |
