...
- `install.html`: step-by-step guided install flow with tool choice, one matching instruction, first prompt, and optional post-use feedback.
...
| `install_flow_step_viewed` | Visitor moves through tool, instruction, prompt, and feedback steps. | Where the guided setup loses people or creates enough momentum to continue. |
| `install_instruction_copied` | Visitor copies a tool-specific install instruction on the install page. | Whether the dedicated guide completes the practical install action. |
| `install_prompt_selected` | Visitor chooses a first-use prompt scenario. | Which activation use case resonates after setup. |
| `install_prompt_copied` | Visitor copies a starter prompt from the install page. | Whether setup leads into first-use activation. |
...
Implementation note: the static pages keep local debug copies in `window.uxrulerLandingEvents` and `window.uxrulerInstallEvents`, push the same events into `window.dataLayer`, dispatch browser events, and send events to PostHog when `window.posthog.capture` is available. PostHog is enabled only on `uxruler.com` production hosts and is configured for pageview/pageleave plus explicit events; autocapture, surveys, replay, flags, heatmaps, dead-click capture, exception capture, and performance capture are disabled to keep the site fast and focused on the decisions above. The footer includes a compact feedback form for early open-ended signals; it records feedback as a landing event and does not write to a repo file, inbox, or separate database. Landing install cards keep direct copy buttons and link to `install.html` for the fuller setup guide. Landing examples let visitors select a simplified starter prompt and inspect the expected action schema before copying. The install page is a guided flow that shows one step at a time: tool choice, one matching instruction, one starter prompt, and optional feedback at the bottom.

Review cadence and decision rule: review PostHog weekly during early release. If visitors enter the install page but do not advance from tool to instruction or prompt, reduce steps or default the path from the referrer or anchor. If visitors open `install.html` but do not copy an install instruction, simplify the instruction for that target. If direct landing copy beats install-page copy, keep the landing fast path prominent. If landing example selections happen but prompt copies lag, simplify the action schema or make the selected prompt more obviously copyable. If instruction copies happen but prompt copies lag, improve the activation prompt step. If footer feedback clusters around unclear value, missing examples, or trust concerns, adjust the page before adding new artifacts. If visitors leave feedback or email, follow up to learn whether UX RULER changed a decision, metric, risk, or next validation step. If language switching is low for several weeks, keep bilingual support but avoid expanding Polish-only content.
